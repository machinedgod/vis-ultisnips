...
ttabtriggerweird   = lpeg.S('!') * (lpeg.R('az') + lpeg.S('?()')) ^ 1 * lpeg.S('!')
ttabtriggerweird2  = lpeg.P('#!')
ttabtrigger        = ttabtriggercomplex + ttabtriggerweird + ttabtriggerweird2 + tlowcasedword
tdescription       = quoted (lpeg.Cg( (tanyprintable - lpeg.S('"'))^1, 'description'))
toption            = lpeg.R('az')

tstartsnippet = lpeg.P('snippet') * tws * lpeg.Cg(ttabtrigger, 'tabtrigger') * tws * tdescription * tws ^ 0 * lpeg.Cg(toption^0, 'options')
tendsnippet   = lpeg.P('endsnippet')
tcontentline  = tanyprintable + lpeg.R(' \t')
tcontent      = ((lpeg.S(' \t') + tanyprintable)^1 - tendsnippet) * tnewline
tsnippet      = tstartsnippet * tnewline * ((tendsnippet * tnewline) + lpeg.Cg(tcontent ^ 1, 'content'))

tcomment  = lpeg.S('#') * tanyprintable^0 * tnewline
tpriority = lpeg.P('priority') * tws * lpeg.Cg(lpeg.S('-')^0 * tdigit^1, 'priority')

-- TODO doesn't work
tsnippetsfile = ((lpeg.Ct(tsnippet) + tpriority + tcomment + tnewline) - -1) ^ 1
